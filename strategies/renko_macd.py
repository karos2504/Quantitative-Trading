...
def _precompute_indicators(df):
    """Merge Renko + MACD + RSI + ATR + slopes into a single DataFrame."""
    ...
    merged.dropna(subset=['MACD', 'Signal', 'bar_num', 'RSI', 'ATR', 'ER'], inplace=True)
...
class RenkoMACDStrategy(Strategy):
    ...
    def next(self):
        ...
        # Require RSI confirmation AND High Efficiency Ratio (ER > 0.3 means low noise context)
        buy_signal = bar >= 2 and macd_val > sig_val and m_slope > s_slope and rsi_val > 50 and er_val > 0.3
        sell_signal = bar <= -2 and macd_val < sig_val and m_slope < s_slope and rsi_val < 50 and er_val > 0.3
...
def _generate_vbt_signals(df):
    """Generate vectorbt entry/exit signals with ER mathematical noise filter."""
